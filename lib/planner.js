...
function attachMainItemIds(main) {
  return (Array.isArray(main) ? main : []).map((item, index) => ({
    ...item,
    id: item?.id || 'main-' + (index + 1) + '-' + sanitizeIdPart(item?.name || item?.detail || 'exercise'),
  }));
}
